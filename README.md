...
- Have three minimum frequencies: "idle," which is the frequency the device will be at when the user stops interacting with it; and "min," which has two frequencies: the first is for initialization, a "fallback" that allows the CPU to ramp to the device's default minimum frequency; and the second is the "immediate boost minimum" frequency for tasks that require immediate performance and cannot wait for higher frequencies. With this synergy, there are three tiers of "minimum frequency," each of which is used in different situations.
