...
- A Scheduler and CPU/GPU only optimization module, without placebo and with total focus on proposing an improved user experience in both efficiency and raw performance.
...
- Integrate the CPU governor optimization strategy called "demand-on-demand," a form of governor optimization like Schedutil or Interactive that seeks a fast response but uses the most efficient frequency possible, scaling as demand increases. Once demand is satisfied, it immediately drops to the lowest frequency, the classic "idle" frequency.
- Optimize GPU boosting in the CAF Boost Framework, allowing the GPU to perform better in certain performance-demanding situations, such as scrolling, and more efficiently. This also aligns the idle time with the display's idle time, aligning both the GPU and Surfaceflinger with the low-power input, allowing for better efficiency and simultaneous performance.
- Optimize some immediate performance demands via CAF Boost Framework, allowing them to have their correct priorities, without overboosting and allowing better "rice" of the main strategy, always looking for a light boosting that allows you to finish the task faster and rest as soon as possible.
