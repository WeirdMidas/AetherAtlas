...
- Optimization of schedtune and uclamp to benefit the scheduler and perform user tasks much better. Details about the Schedtune & Uclamp optimization below:
  - Prioritizes the scheduling of processes between idle cores for foreground tasks and mainly, top-app.
  - Always limit the background. Have almost total control over the background to avoid wearing out the cores with "trivial" tasks.
  - If the user has one of these two methods, he can notice a huge gain in terms of scheduling and prioritization of tasks between cores. Users with Uclamp or Schedtune perform more responsively than users without one of the two (as occurs in generic kernels such as the classic CFS standard).
