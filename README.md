...
- Optimize Boost Framework boosting. Eliminate unnecessary boosting and focus only on those that can improve the device's fluidity while minimizing the impact on power savings. The SOC's Boost Framework will be used to provide immediate performance in these situations, focusing on efficiency (quickly completing the task and resting immediately).
- Respect the way the device architecture works, preventing all cores from scaling to higher frequencies for light to moderate tasks, allowing the CPU to scale as needed. This reduces migration failures, respects the capabilities of each core, and makes hotplugging more efficient overall.
- Group as many tasks as possible onto small cores, use small cores for lightweight and unimportant tasks, and only allow the more powerful cores to be used for tasks that deserve to be boosted. This saves energy by improving the way the scheduler, tracker, and governor interact with user tasks.
