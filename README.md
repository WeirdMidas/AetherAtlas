...
- Replace some "static" optimizations (such as those via sysfs) with hints/profiles. This is better because it integrates the optimizations with the Android framework, making everything more coherent and less invasive to the ROM. Maximizing the efficiency that the user would achieve through these optimizations, in addition to, of course, reducing the chances of crashes and such.
- Optimize the way the task scheduler distributes tasks according to the SOC architecture. big.LITTLE receives more attention by preferring small cores for tasks that can satisfy them, grouping tasks together when sending them to big cores, allowing big cores to provide support if needed by little cores. DynamLQ is preferred, a load balancing approach that has no preferences, but rather specializes in leveraging the device's capabilities without "unnecessary" things like grouping, etc.
