...
- Prioritize a scheduling method that respects the way Android works. Having the style of: input (feed subsequent tasks) > scheduler (with the energy that the input designed it for, organizes and reorganizes tasks quickly) > governor (maintains or increases the frequency), reducing the task start power noticeably. In addition, there are some extras in this scheduling that are explained below:
  ...
  - Use thread affinity correctly, allowing certain jobs to scale efficiently between cores for better performance and less unnecessary migration overhead.
- Use the CPUset more intelligently and respectfully with Android's dynamic workload. Reserve one (or two if the device is a 6x2) big cores for top-app and foreground tasks, along with reserving two small cores for background and top-app tasks. Based on this, fix the launcher&home on all cores and make the foreground use the small cores when the user is scrolling and make it use cores 0-5 when an app is opened to increase app startup speed by up to 4%, but only in these situations, in addition to allowing the foreground to use the big cores if the user is doing other tasks. Allowing multithreading focused more on efficiency than raw performance, saving energy when using messenger or social media apps and maintaining foreground performance in apps that need it.
...
- Optimize the media in general. Reduce the energy consumption caused by audio, encoder and improve the quality and agility of the camera. As the media in general directly impacts the UX for more common users, perfd ​​opt now optimizes these aspects, favoring less use of resources that could be avoided, improving the execution and efficiency of audio, video and camera, which translates into slightly improving the user experience in this aspect of the Android system.
  - Instead of using generic forms of network management, audio, etc. as in AOSP ROMs. Use forms integrated with Snapdragon processors, favor compatibility and reduce power consumption by using solutions that understand the device better.
- Along with media optimization, you will also have Wifi/Network optimizations for better overall network consistency. Less bufferbloat and other improvements that allow for above-normal ping, stability and network speed.
- Use Matt Yang's Perfd-opt tweaks in other ways. Like LPM BIAS HYS which is applied via perf hint instead of sysfs, this generally reduces the impact on power consumption and even improves energy efficiency by preventing the CPU from staying on for too long unnecessarily when the device goes to idle, only keeping it on when the user is actually using the device. This generally reduces idle consumption of profiles like performance and even fast, for example.
